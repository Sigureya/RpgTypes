...
// const gggg3 = (plugin: PluginSchemaArray, kinds: ReadonlyArray<ParamKinds>) => {
//   const singleKinds: Set<ParamKinds> = new Set(kinds);
//   const arrayKinds = new Set(kinds.map((k): `${ParamKinds}[]` => `${k}[]`));

//   const structs = filterStructsEx(
//     plugin.structs,
//     (p): p is PluginParam<PrimitiveParam> => true
//   );
//   const structNames = new Set(structs.map((s) => s.struct));
//   const params = filterParams(plugin.params, structNames, (p) => {
//     return singleKinds.has(p.kind); // || arrayKinds.has(p.kind);
//   });
