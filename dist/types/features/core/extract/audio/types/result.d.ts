...
export type GameDataAudioExtractor = EventContainerAudioExtractor & EventCommandAudioExtractor;
...
export interface EventCommandAudioExtractor {
    extractAudioCommands(commands: ReadonlyArray<EventCommand>): AudioCommandParameter[];
}
export interface ExtractedBattleEventAudio<T extends AudioPluginCommandParameter = AudioPluginCommandParameter> {
    eventId: number;
    commands: (T | AudioCommandParameter)[];
    pageIndex: number;
}
export interface ExtractedCommonEventAudio<T extends AudioPluginCommandParameter = AudioPluginCommandParameter> {
    eventId: number;
    commands: (T | AudioCommandParameter)[];
}
export interface ExtractedMapAudios<T extends AudioPluginCommandParameter = AudioPluginCommandParameter> {
...
export interface ExtractedMapEventAudios<T extends AudioPluginCommandParameter = AudioPluginCommandParameter> {
    eventId: number;
    name: string;
    pageIndex: number;
    commands: (T | AudioCommandParameter)[];
